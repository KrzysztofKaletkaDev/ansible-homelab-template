...
- Optionally, list a wall kiosk's address in `camera_wall_trusted_ips` (e.g. `["10.0.0.69"]`) to let it skip the login — only the login: the path allowlist and the final 403 still apply. The address must be pinned by a DHCP reservation, and it must be a LAN client address, never a Docker one; the `caddy` role refuses Docker gateways and `172.16.0.0/12`. See [ADR-0012](docs/adr/0012-camera-wall-login-exception-by-client-address.md).
